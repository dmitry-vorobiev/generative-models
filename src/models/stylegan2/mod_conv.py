...
class ModulatedConv2d(_ConvNd):
    ...
    def _modulate(self, weight: Tensor, style: Tensor) -> Tensor:
        w = weight[None, :]  # Add batch dim
        # Add out channels dim: 1 - normal, 2 - upsample
        s = style.unsqueeze(1 + int(self.upsample))
        s = s[:, :, :, None, None]
        return w * s

    @staticmethod
    def _demodulate(w: Tensor, eps=1e-8) -> Tensor:
        d = torch.rsqrt(w.pow(2).sum(dim=(2, 3, 4), keepdim=True) + eps)
        return w * d
    ...
    def conv2d_forward(self, x, style, weight, bias):
        ...
        if self.upsample:
            ...
            out = _upfirdn_2d_ref(out, self.lp_weight, upx=1, upy=1, downx=1, downy=1,
