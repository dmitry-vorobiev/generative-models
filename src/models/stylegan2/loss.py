...
def r1_penalty(real_pred: Tensor, real_img: Tensor):
    grad, = autograd.grad(real_pred.sum(), real_img, retain_graph=True)
...
class D_LogisticLoss_R1(nn.Module):
    ...
    def forward(self, real_pred: Tensor, fake_pred: Tensor, real_img: Tensor) -> Tensor:
        loss = F.softplus(fake_pred) + F.softplus(-real_pred)

        if self.count % self.freq == 0:
            penalty = r1_penalty(real_pred, real_img)
...
class G_LogisticNSLoss_PathLenReg(nn.Module):
    ...
    def __init__(self, pl_ema_decay=0.01, pl_reg_freq=4, pl_reg_weight=2.0):
        ...
        self.register_buffer('path_len_avg', torch.zeros(1))
        self.count = 0

    def forward(self, fake_pred: Tensor, fake_img: Tensor, fake_w: Tensor) -> Tensor:
        loss = F.softplus(-fake_pred)

        if self.count % self.freq == 0:
            penalty, self.path_len_avg = path_len_penalty(
                fake_img, fake_w, self.path_len_avg, self.decay)
